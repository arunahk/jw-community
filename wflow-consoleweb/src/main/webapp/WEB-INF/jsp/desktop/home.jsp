            ...
            success:function(data) {
                ...
                for( var i=0; i<apps.length; i++) {
                    ...
                    for( var j=0; j<userviews.length; j++){
                        ...
                        content += '<a class="app-link" target="_blank" href="' + baseUrl + uv.url + '">\
                                            ...
                                            <div class="uv-description" style="display:none">' + UI.escapeHTML(uv.description) + '</div>\
                                        </a>\
                                    </li>';
                    }
                }
                ...
                <c:if test="${isAdmin}">
                    if (!isMarketplace) {
                        ...
                        var marketplaceAppPopupDialog = new PopupDialog(contextPath + "/web/console/home", " ");
                        $("#marketplace-apps a.app-link").click(function(e) {
                            ...
                            var appDescription = $(this).find(".uv-description").text();
                            var appDownloadUrl = "";
                            var idx = appDescription.indexOf("[http");
                            if (appDescription && idx >= 0) {
                                appDownloadUrl = appDescription.substring(idx+1, appDescription.indexOf("]"));
                            }
                            var marketplaceAppUrl = contextPath + "/web/desktop/marketplace/app?url=" + encodeURIComponent(appUrl) + "&appId=" + encodeURIComponent(appId) + "&name=" + encodeURIComponent(appName) + "&download=" + encodeURIComponent(appDownloadUrl);
                            marketplaceAppPopupDialog.src = marketplaceAppUrl;
